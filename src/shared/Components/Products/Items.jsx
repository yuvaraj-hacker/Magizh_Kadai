...
const Items = (prpos) => {
  const { isLoading, products, placements, handleAddToCart, handleAddToWishlist, setSort, wishlistData, scrolled } = prpos;
  ...
  return (
    <>
      {/* products */}
      <div className={`w-full sticky top-[50px] lg:top-[75px] z-40 ${scrolled?'shadow-lg':''}`}>
        <div className="sticky top-[95px] lg:top-[82px] bg-white dark:bg-black z-40 flex items-center justify-between w-full px-4 pb-2 pt-5 lg:pt-10 lg:px-5">
          <div className="mx-5 lg:mx-10 dark:text-black dark:bg-white dark:p-2 dark:rounded-3xl">{`${products.length} results`}</div>
          {/* Select */}
          ...
          <Select labelPlacement={placements[0]} size='sm' 
            ...
            className="w-40 max-w-xs lg:w-60 "
            classNames={{
              trigger: " dark:bg-gray-100 dark:text-black",
              listbox: " dark:bg-gray-200 dark:text-black",
              popover: " dark:bg-gray-200 dark:text-black"
            }}
          >
      ...
      <div className="relative p-2 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 overflow-y-auto 2xl:grid-cols-6 xl:grid-cols-5 gap-x-3 md:px-5 ">
        {products.map((prod, i) => (

             <div key={i} className="relative group ">
                            <div className="w-full h-[300px]  sm:min-h-[380px] sm:max-h-[380px]  bg-white flex justify-between flex-col relative mb-5 shadow-md border rounded-2xl hover:shadow-xl duration-300 hover:-translate-y-1 active:scale-95">
                          ...
                          {prod.QTY > 0 && prod.QTY !== null && (
                              <button onClick={(e) => { e.preventDefault(); handleAddToCart(prod); }} className="flex justify-center items-center border-[1.5px] rounded-full bg-gray-100 hover:bg-white  group overflow-hidden shadow-md duration-300" >
                               ...
                               <button onClick={() => handleAddToWishlist(prod)} className="  group">
                              ...
                              <Link to={`/product-details/${prod._id}`} state={{ product: prod }}>
                                ...
                                <div className="relative flex items-center justify-center mx-3 lg:m-5 sm:h-[250px] h-[170px] overflow-hidden rounded-lg"> 
                            ...
                            <div className="p-5 pt-0 ">
                              <h2 className="mt-3 text-sm text-black dark:text-white md:text-base line-clamp-1 text-left">
                                {prod.Product_Name}
                              </h2>
                            ...
                            <>
                              ...
                              {/* Original Price */}
                              <h3 className="text-sm text-gray-400 line-through dark:text-white">
