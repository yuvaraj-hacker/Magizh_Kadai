  ...
  return (
    <div className="relative flex items-center justify-center mx-3 lg:m-5 sm:h-[250px] h-[170px] overflow-hidden rounded-lg">
      {imageArray.map((image, index) => (
        <img key={`${productId}-${index}`} src={`${apiurl()}/${image}`} alt={`Product ${index + 1}`} className={`${index === 0 ? 'block' : 'hidden'}  object-contain group-hover:scale-110  group-hover:opacity-80 duration-300  rounded-lg`} />
      ))}
    </div>
  );
};
    ...
    <>
     <article className="rounded-3xl flex flex-col overflow-hidden relative">
      <div className=" absolute w-full grid grid-cols-12 grid-rows-12 rounded-3xl overflow-hidden h-full">
        <div className="lg:col-span-8 lg:row-span-12 row-span-7 order-2 lg:order-1 col-span-12 bg-gradient-to-br from-[#ecf5e1] to-primary_green h-full"></div>
        <div className="lg:col-span-4 lg:row-span-12 row-span-5 order-1 lg:order-2 col-span-12 bg-primary h-full"></div>
      </div>
      <div className="relative z-10 flex justify-between w-full max-w-screen-xl mx-auto py-5 lg:py-14 px-3 2xl:px-0">
        <div className="flex flex-col gap-1 lg:gap-2">
          <p className="capitalize tracking-widest text-white lg:text-black">New collection</p>
          ...
          >
            {Product.slice(0,7).map((prod, i) => (
              <SwiperSlide key={prod._id || i} className="">
                <motion.div variants={slideVariants} className="relative group ">
                  <div className="w-full h-[300px]  sm:min-h-[380px] sm:max-h-[380px]  bg-white flex justify-between flex-col rounded-2xl relative mb-5 border shadow-md hover:shadow-lg duration-300 active:scale-95">
