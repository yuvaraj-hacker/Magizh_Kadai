...
{Object.keys(groupedProducts || {}).map((category, index) => (
  <div key={index} className="mb-6">
    <div className="flex justify-between items-center my-2">
      ...
      <Link to={`products?category=${category}`} className="h-fit sm:h-full   p-2 lg:px-4 rounded-full border border-primary flex gap-2 justify-center items-center  group/vwbtn *:duration-300"><button className="text-sm lg:text-base">View Category</button></Link>
    </div>
    <div className="">
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2 ">
      ...
      {groupedProducts[category].slice(0, 6).map((product, idx) => (
        <div key={idx} className="flex flex-col group duration-300 p-2 lg:p-3 from-[#70c2a9] via-primary to-primary hover:to-[#8ad1bc] hover:via-primary hover:from-primary rounded-2xl border hover:border-primary hover:bg-gray-100">
          <div className="border h-40 md:h-60 overflow-hidden rounded-xl flex items-center justify-center bg-white">
